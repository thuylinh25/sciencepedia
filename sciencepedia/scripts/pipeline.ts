...
function buildPrompt(count: number, dryRun: boolean): string {
  return [
    ...
    "",
    "Có HAI chỗ chứa việc dở, và phải xem cả hai. Xem chỗ nào trước cũng được,",
    "nhưng bỏ sót một chỗ là mất việc.",
    "",
    "**1. CSDL** — chạy `npm run publish:check -- --draft`. Bài nào đang ở DRAFT",
    "là việc dở của một lượt bị cắt giữa chừng.",
    "",
    "**2. File trong `docs/content/drafts/`** — bản nháp có file nhưng KHÔNG có",
    "bài tương ứng trong CSDL là việc dở đã chết trước bước 10. Đây là chỗ mà",
    "phép kiểm CSDL không bao giờ thấy.",
    "",
    "Vì sao phải xem chỗ thứ hai: hàng CSDL chỉ được tạo ở bước 10, trong khi",
    "bước 1–9 chỉ để lại file. Ngày 2026-09-06 một lượt chạy 44 phút hết hạn mức",
    "ở bước 3 của chủ đề `dien-tich-va-dong-dien`; bản nháp và claim map nằm trên",
    "đĩa runner, CSDL trống trơn, nên lượt sau kết luận \"không có việc dở\" rồi",
    "lấy chủ đề mới. $17 và 44 phút đổ đi.",
    "",
    "Làm nốt việc dở TRƯỚC khi lấy chủ đề mới khỏi hàng đợi. Ưu tiên tuyệt đối,",
    "vì hàng đợi bỏ qua dòng nào ĐÃ có bài — một bản nháp bị bỏ quên sẽ không",
    "lượt nào nhặt lại, nó thành bài mồ côi vĩnh viễn.",
    "",
    "Với bản nháp chỉ có file: đọc `<slug>.meta.yaml` cạnh nó để biết đã xong tới",
    "bước nào. File meta ghi rõ, và nó cũng ghi thứ gì đã mất không cứu được.",
    "",
